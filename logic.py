...
skills = [ (i,) for i in (['Python', 'SQL', 'API', 'Telegram'])]
statuses = [ (i,) for i in (['На этапе проектирования', 'В процессе разработки', 'Разработан. Готов к использованию.', 'Обновлен', 'Завершен. Не поддерживается'])]
...
def add_new_column(table_name='projects', new_column_name='photo', new_column_type='TEXT'):
    conn = sqlite3.connect('projects.db')
    
    with conn:
        execute_alter = f"ALTER TABLE {table_name} ADD COLUMN {new_column_name} {new_column_type}"

        conn.execute(execute_alter)
        conn.commit()
